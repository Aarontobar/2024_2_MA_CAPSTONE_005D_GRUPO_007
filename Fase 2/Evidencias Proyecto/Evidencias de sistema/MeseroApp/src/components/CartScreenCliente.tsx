// src/components/CartScreenCliente.tsx
...
const CartScreenCliente = ({ route, navigation }: { route: any, navigation: any }) => {
  ...
  const confirmOrder = () => {
    ...
    const url = getApiUrl(`menu/generar_pedido_app_cliente.php?mesa_id=${mesaId}&id_mesero=${userId}&total=${totalPrice}`);
    const carritoParams = carrito.map(item => `carrito[]=${item.id_platillo},${item.quantity},${item.precio}`).join('&');

    axios.post(`${url}&${carritoParams}`)
      .then(response => {
        const data = response.data as { success: boolean; id_pedido?: number; message?: string };
        if (data.success && data.id_pedido) {
          Alert.alert('Pedido confirmado', 'Tu pedido ha sido confirmado.');
          navigation.navigate('VerPedido', { id_pedido: data.id_pedido });
        } else {
...
export default CartScreenCliente;